...
def _T_kepler(de, f, M):
    """Computes the period of a keplerian orbit corresponding to a burst 
    in units of seconds.
    ...
    """
    e = 1-de #eccentricity
    r_p = _rp_kepler(de, f, M) #periastron distance in units of [M]
    semi_a = r_p*((1+e)/(1-e)) #semi major axis in units of [M]
    
    T = np.sqrt(4*np.pi**2*semi_a**3)*_Tsun #Kepleraian period in units of [s]
    return T

class EccPrior(object):
    ...
    def get_logprior(self, tf_from_BW, Mtot, Mc, destar, tstar, fstar):
        ...
        dt0 = 0.2*_T_kepler(destar, fstar, Mtot)
        df0 = fstar/(8*np.pi)
        dde0 = destar/10
